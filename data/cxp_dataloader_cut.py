...
class CXPDataset(Dataset):
    def __init__(
        self, root_dir, transforms, mode, file_name=None, stage="None", args=None
    ) -> None:
        ...
        print(self.df.columns[5:])
        print("num labels:", len(self.df.columns[5:]))
        label_cols = list(self.df.columns[5:])
        print("label cols:", label_cols)
        print("No Finding idx:", label_cols.index("No Finding") if "No Finding" in label_cols else None)
        print("No Finding positives:", (self.df["No Finding"].values == 1).sum() if "No Finding" in self.df else "NA")
        print("all-zero rows (over selected disease cols):", (self.gt.sum(axis=1) == 0).sum())
        class_counts = (self.gt == 1).sum(axis=0)
        for i, name in enumerate(label_cols):
            print(i, name, class_counts[i])
        print("max:", label_cols[int(class_counts.argmax())], class_counts.max())
