...
class Chest_PDC_Dataset(Dataset):
    ...
    def __init__(
        ...
        self.pdc_clean_label = pdc_dataset.clean_gt
        ...
        self.nih_length = len(self.nih_label)
        self.pdc_length = len(self.pdc_label)
        gt_category_counts, noisy_category_counts = pdc_dataset.get_category_counts()
        for category, count in gt_category_counts.items():
            print(f"gt Category {category}: {count} samples")
        for category, count in noisy_category_counts.items():
            print(f"noisy Category {category}: {count} samples")
    ...
    def load_nih(
        ...
        self.drop_list = []
        df_path = os.path.join(self.root_dir_nih, "Data_Entry_2017_v2020.csv")
    ...
    def _compute_category_counts(self):
        # 私有方法，用于计算类别样本数量
        print(self.knn[self.nih_length:].shape) 
        #label_indices_list = [np.where(label_array == 1)[0].tolist() for label_array in labels]
        #flattened_labels = label_indices_list.flatten()
        # 将二维标签数组转换为一维标签索引列表
        label_indices = []
        for label_array in self.knn[self.nih_length:]:
            label_array = np.where((label_array / 10) > 0.3, 1, 0)
            label_indices.extend(np.where(label_array == 1)[0])  # 使用extend而不是append来展平列表
        return Counter(label_indices)
   
    def get_category_counts_labels(self):
        # 公共方法，返回类别样本数量的统计信息
        # 如果在初始化时没有计算，则在这里计算并缓存结果
        if not hasattr(self, 'category_counts'):
            category_counts = self._compute_category_counts()
        return category_counts
    
    def compute_auc(self, class_num = 14):
        print(self.train_label.shape, self.pdc_clean_label.shape, self.knn[:,0].shape)
        train_label = np.transpose(self.train_label[self.nih_length:], axes=(1, 0))
        pdc_clean_label = np.transpose(self.pdc_clean_label, axes=(1, 0))
        knn = np.transpose(self.knn[self.nih_length:], axes=(1, 0))
        print(train_label.shape, pdc_clean_label.shape, knn[:,0].shape)
        print(knn[:,0])
        noise_all_auc = [
                roc_auc_score(train_label[:, i], pdc_clean_label[:, i])
                for i in range(class_num - 1)
            ]
        noise_auc = np.asarray(noise_all_auc).mean()
        correct_all_auc = [
                roc_auc_score(np.where((knn[:,i] / 10) > 0.3, 1, 0), pdc_clean_label[:, i])
                for i in range(class_num - 1)
            ]
        correct_auc = np.asarray(correct_all_auc).mean()
        return noise_auc, correct_auc
